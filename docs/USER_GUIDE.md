...
审计事件覆盖账户创建、充值、转账、退款、冲正、对账导入、执行对账和处理差异。事件包含时间、操作人、业务对象、结果、摘要和关联标识。审计事件只追加、不可修改；重复提交同一幂等请求不会额外写入重复事件。

## 6. 自动对账

进入**自动对账**（`/admin/reconciliation`），导入一个 `SYNTHETIC_CHANNEL` 的 UTF-8 CSV。固定表头为：
...
导入成功后批次为 `IMPORTED`，点击**开始对账**。系统只把成功充值作为渠道候选，结果类型如下：
...
管理员可为未关闭差异填写备注并处理。处理会写入独立的不可变历史，**原支付和账本事实不会被修改**；已匹配结果不能处理。相同文件的 SHA-256 digest 具备导入幂等性，重复上传返回原批次。
